...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        let realm = try! Realm()
        //文字列によるクエリ
        if realm.objects(Diary.self).filter("date == '\(self.date!)'").last == nil {
            print("データはありません")
            ...
            let component = DateComponents(calendar: Calendar.current, /*year: 2019, month: 6, day: 6, */hour: 21, minute: 00)
